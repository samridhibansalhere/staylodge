...
async function BookRoomPage({params}:{params:{id:string;};}) {
  ...
  return (
    <div>
        <div>
        <h1 className='font-bold text-gray-500 text-2xl'>{room.name} - {room.hotel.name}</h1>
        <span className='text-sm text-gray-500'>{room.hotel.address}</span>
        </div>
        <div className='grid grid-cols-1 lg:grid-cols-3 mt-10'>
            <div className='col-span-2'><RoomInfo room={room}/></div>
