  ...
  return (
    <div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-7 mt-7 capitalize'>
        ...
        {renderRoomProperty("Phone Number",room.hotel.phone)}
        {renderRoomProperty("Owner Name",room.hotel.owner)} 
        {renderRoomProperty("Email",room.hotel.email)}   
      </div>
      <div className='mt-7'>
        <h1 className='text-xs text-gray-600'>Amenities</h1>
      <div className='flex flex-wrap mt-2 gap-7'>
        {room.amenities.split(",").map((amenity,index)=>(
            <div key={index} className='bg-gray-200 text-gray-600 rounded-md px-3 py-1 text-xs capitalize'>
                {amenity}
            </div>
        ))}
      </div>
